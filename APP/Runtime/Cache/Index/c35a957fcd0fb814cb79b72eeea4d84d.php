...
<div style="width: 90%;margin-left: 5%;margin-top: 20px;overflow:hidden;border-radius:5px">
    ...
    <p id="chushoubg"onclick='showhidediv("chushou")'style="float: left;width: 25%;text-align: center;background-color: #87CEEB;height: 30px;line-height: 30px;color:#fff">我的卖出</p>
    <p id="zzjybg" onclick='showhidediv("zzjy")'style="float: left;width: 25%;text-align: center;background-color: #87CEEB;;height: 30px;line-height: 30px;color:#fff">正在交易</p>
    <p id="ywcjybg" onclick='showhidediv("ywcjy")'style="float: left;width: 25%;text-align: center;background-color: #87CEEB;;height: 30px;line-height: 30px;color:#fff">历史记录</p>
</div>
...
<div id="chushou" style="-left: 5%;margin-bottom:80px;margin-top:30px;display: none;width: 90%; margin-left: 5%;">
    ...
    <div>
        <?php if(is_array($cslkb)): foreach($cslkb as $key=>$v): ?><ul class="myjy_body"style="width:100%;margin-left:0%;margin-top:10px">
                ...
                <li style="width: 24%;"><?php echo ($v["lkb"]); ?></li>
                <li style="width: 20%;" class="money"><?php echo ($v["jb"]); ?></li>
                <li style="width: 20%;"><a href="<?php echo U('Emoney/del',array('id'=>$v['id']));?>" style="width: 90%;margin-left: 5%;display: block;height: 30px;line-height: 30px;text-align: center;background-color: #87CEEB;border-radius: 5px;color: #FFFFFF">取消</a></li>
                <li style="width: 18%;">
                    <?php if($v["zt"] == 0): ?><a href="#" style="width: 90%;margin-left: 5%;display: block;height: 30px;line-height: 30px;text-align: center;background-color: #;border-radius: 5px;color: #FFFFFF">未交易</a><?php endif; ?>
                    <?php if($v["zt"] == 1): ?><a href="#" style="width: 90%;margin-left: 5%;display: block;height: 30px;line-height: 30px;text-align: center;background-color: #87CEEB;border-radius: 5px;color: #FFFFFF">交易中</a><?php endif; ?>
                    <?php if($v["zt"] == 2): ?><a href="#" style="width: 90%;margin-left: 5%;display: block;height: 30px;line-height: 30px;text-align: center;background-color: #87CEEB;border-radius: 5px;color: #FFFFFF">交易完成</a><?php endif; ?>
                </li>
            </ul><?php endforeach; endif; ?>
...
<div id="zzjy" style="display:none;margin-bottom:80px;">
 ...
 if($v['datatype']=='cslkb'){ ?>
	 ...
	 <div style="width:90%;margin-left:5%;padding-top:30px;font-size:14px">
		<P>交易提示：</P>
		<P>
           <span style="color: red"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;请买家在30分钟内完成付款并上传截图，卖家在收到款后30分钟内完成交易。否则对方可以投诉，有过错一方将会受到冻结余额或者封号处罚。
           </span>
               <br/>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;为保护您的资金安全，请使用您个人资料内绑定的支付方式向对方绑定的收款账号内付款，付款后请上传付款凭证图片，如使用绑定账户以外的账户交易，系统无法监管，会检测为虚假交易。</P>
	</div>
</div>
    ...
    $(".btnmaijia").bind("click",function() {
        ...
        if(oid==""){
            ...
            $.ajax({
                ...
                success: function (data) {
				console.log(data);
                    var pay_method="<br>支付宝户名："+data.alipay_user_name+"<br/>";
                    pay_method +=  "<br>支付宝收款码：<img width='60' height='60' src='"+data.alipay_code_img+"'<br/>";

                    pay_method +="<br>微信号："+data.weixin+"<br/>";
                    pay_method +="<br>微信号户名："+data.wechat_user_name+"<br/>";
                    pay_method +=  "<br>支付宝收款码：<img width='60' height='60' src='"+data.wechat_code_img+"'<br/>";

                    pay_method +="<br>银行卡号："+data.brank_num+"<br/>";
                    pay_method +="<br>属于银行："+data.brank_name+"<br/>";
                    pay_method +="<br>户主名："+data.brank_user_name+"<br/>";

                    $.modal({
                        title: data.name,
                        text: "<div style='text-align:left'><i class='fa fa-usd' style='color: #00FF00;width: 12px;margin:0 auto;font-size: 1.2em;margin-left: -2px;'></i>："+data.meiyuan+"<br/>"+"<i class='fa fa-jpy' style='color: #FF0000;width: 12px;margin:0 auto;font-size: 1.2em;margin-left: -2px;'></i>："+data.rmb+"<br>"+"手机号："+data.mobile+"<a href='tel:"+data.mobile+"'><input type='button' value='拨打电话' style='margin-left:5px; padding:3px 7px; border:0px; border-radius:4px; color:#ffffff; background:#09F;'></a><br>"+"姓名："+data.truename+"<br>支付宝账号："+data.zhifubao+"<br/>"+pay_method+"<font style='color:#F00;'>友情提示：请认真核对交易信息，或者电话沟通确认交易！！</font></div>",
                        buttons:[
                            { text: "确定", className: "default", onClick: function(){ } },
                        ]
                    })
        ...
        if(oid==""){
            ...
            $.ajax({
				...
				console.log(data);
                    $.modal({
                        title: data.name,
                        text: "<div style='text-align:left'><i class='fa fa-usd' style='color: #00FF00;width: 12px;margin:0 auto;font-size: 1.2em;margin-left: -2px;'></i>："+data.meiyuan+"&nbsp;&nbsp;<br/>"+"<i class='fa fa-jpy' style='color: #FF0000;width: 12px;margin:0 auto;font-size: 1.2em;margin-left: -2px;'></i>："+data.rmb+"&nbsp;&nbsp;<br>"+"手机号："+data.mobile+"<a href='tel:"+data.mobile+"'><input type='button' value='拨打电话' style='margin-left:5px; padding:3px 7px; border:0px; border-radius:4px; color:#ffffff; background:#09F;'></a><br>"+"姓名："+data.truename+"<br>" +
                        "<font style='color:#F00;'>友情提示：请认真核对交易信息，或者电话沟通确认交易！！</font></div>",
