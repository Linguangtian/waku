 ...
 if(C('recharge_is') ==1){ ?> -->
		...
		<?php if(is_array($typeData)): foreach($typeData as $key=>$ty): ?><li style="position:relative;">
				...
				<div style="width:60%;display:inline-block;">
                    ...
                    <p>兑换单价：<?php echo ($ty["price"]); ?>CHT</p>
					<P>产量/1小时：<?php echo ($ty["shouyi"]); ?></P>
					<P>矿机算力：<?php echo ($ty["gonglv"]); ?>GH/s</P>
					<P>运行周期：<?php echo ($ty["yszq"]); ?>小时</P>
				</div>
					 <a href="<?php echo U('Index/Shop/pcontent',array('id'=>$ty['id']));?>" style="color: #fff;margin-top: 0px;display:block;position:absolute;right:10px;top:50%;margin-top: -15px;font-size: 16px;padding: 5px;background-color: skyblue; color: #fff; border: 0px solid #fff;border-radius: 4px;">兑换</a>
