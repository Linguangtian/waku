...
<!-- <div class="box-body chart-responsive" style="background-image: url('__PUBLIC__/user2/images/kx.png');"> -->
                    <div id="chart" style="width:100%;height:80px;"></div>
            </div>
        </div>
    </div>
</section>
	...
	<div style="width: 90%;margin-left: 5%;margin-top: 20px;overflow:hidden;border-radius:5px">
		<p id="qiugou_list"onclick='showhidediv("qiugou")'style="float: left;width: 50%;text-align: center;background-color:#87CEEB;height: 30px;line-height: 30px;color:#fff;border-bottom-left-radius: 5px;border-top-left-radius: 5px">买入CHT</p>
		<p id="chushou_list" onclick='showhidediv("chushou")'style="float: left;width: 50%;text-align: center;background-color: #fff;height: 30px;line-height: 30px;border-top-right-radius: 5px;border-bottom-right-radius: 5px">卖出CHT</p>
	</div>


	<div id="qiugou" style="padding-bottom:50px;background-color:#fff;margin-top:0px;width:100%;margin-left:0%">
		<div style="color:red;line-height: 30px;font-size: 15px; height: 30px;text-align: center">只显示单价最高10张单</div>
		<form  id="qiugou" class="mmform" style="margin-top: 0px" action="/Index.php/Home/Info/myjiaoyis" method= "POST" onsubmit="return false" >
 ...
 style="width:60%"/>
        ...
        <!-- <span class="fr" id="count_down2" onClick="send_sms_reg_code2()" style="background-color: #87CEEB; float:right;display: inline-block; height:30px; line-height:30px;width: 33%;text-align: center;font-size: 1em;padding: 0px 0 0px;border-radius: 5px">发送验证码</span> -->


		<button type="submit" id="mmm" style="float:right;width:33%;margin-top:-25px;background-color:#87CEEB;height: 30px;line-height: 30px;background-color:#87CEEB;height: 30px;line-height: 30px;border-radius:5px;border:0px;color:#fff">买入</button>
	</form>

     <div style="width:90%; margin:0 auto;">
    	<form action="<?php echo U('index/emoney/index');?>" method="get">
     <!--  <input type="text" name="p_id" style=" width:60%;" placeholder="请输入会员手机号">
       	   <input type="submit" style=" width:33%;height: 30px; float:right;line-height: 30px;background-color:#87CEEB;height: 30px;line-height: 30px;border-radius:5px;border:0px;color:#fff; text-align:center;" value="查找用户">
        -->
	...
	<table class="jyzx_list" style="margin-top:20px;margin-bottom:80px;border-collapse:collapse;table-layout: fixed">
		...
		<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr style="margin-bottom:100px">
				...
				<?php if($v["zt"] == 0): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
					<button style="background-color: 	#87CEEB;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" class="btnchushou">点击卖出</button>
				</td><?php endif; ?>
				...
				<?php if($v["zt"] == 2): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
					<button style="background-color: 	#87CEEB;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" class="wancheng">交易完成</button>
				</td><?php endif; ?>
	...
	<div style="color:red;line-height: 30px;font-size: 15px; height: 30px;text-align: center">只显示单价最低10张单</div>

	<div id="chushou" style="display: none; padding-bottom:50px;background-color:#fff; position:relative;" >
	<form id="qiugou" class="nnform" style="margin-top: 0px" action="/Index.php/Home/Info/cslkb" method= "POST" onsubmit="return false">
       ...
       <!-- <input type="text" name="password_n" id="password_n" placeholder="请输入手机验证码"style="width:60%"/> -->
        <!-- <span class="fr" id="count_down" onClick="send_sms_reg_code()" style="background-color: #87CEEB; float:right;display: inline-block; height:30px; line-height:30px;width: 33%;text-align: center;font-size: 1em;padding: 0px 0 0px;border-radius: 5px">发送验证码</span> -->
        ...
        -->
		<button type="submit" id="nnn" style="float:right;width:33%;margin-top:-22px;background-color:#87CEEB;height: 30px;line-height: 30px;background-color:#87CEEB;height: 30px;line-height: 30px;border-radius:5px;border:0px;color:#fff">卖出</button>
	</form>
		...
		<table class="jyzx_list" style="margin-top:20px;margin-bottom:80px;border-collapse:collapse;table-layout: fixed">
			...
			<?php if(is_array($lists)): foreach($lists as $key=>$v): ?><tr style="margin-bottom:100px">
					...
					<?php if($v["zt"] == 0): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
							<button style="background-color: 	#87CEEB;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" class="btnsc">点击买入</button>
						</td><?php endif; ?>
					<?php if($v["zt"] == 1): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
							<button style="background-color:#666;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" >交易中</button>
						</td><?php endif; ?>
	...
	<script>
		function showhidediv(id) {
			...
			if (id == "qiugou") {
				...
				qiugoubg.style.backgroundColor = " 	#87CEEB"
				...
				zhengzaibg.style.backgroundColor = " 	#87CEEB";
        ...
        $.ajax({
            ...
            success:function(data){
				...
				var html='';
					...
					//html+='<span class="fr" id="count_down2" onClick="send_sms_reg_code2()" style="background-color: #87CEEB; display: inline-block; height:30px; line-height:30px;width: 29%;text-align: center;font-size: 1em;padding: 0px 0 0px;border-radius: 5px; color:#ffffff;margin-left:5px;">发送</span>';
...
$(function(){
        $("#nnn").bind("click",function(){
           ...
           var html='';
					...
					//html+='<span class="fr" id="count_down" onClick="send_sms_reg_code()" style="background-color: #87CEEB; display: inline-block; height:30px; line-height:30px;width: 29%;text-align: center;font-size: 1em;padding: 0px 0 0px;border-radius: 5px; color:#ffffff;margin-left:5px;">发送</span>';
