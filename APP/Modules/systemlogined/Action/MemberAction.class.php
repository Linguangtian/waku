	...
	class MemberAction extends CommonAction{
		...
		public function chekuserHandle(){
			...
			if($data['checkstatus']==3){
				...
				/*M('member')->where(array('username'=>$info['parent']))->setInc("parentcount");
				$path = explode('|', $info['parentpath']);
				...
				foreach ($parent_list as $v) {
					M('member')->where(array('id'=>$v['id']))->setInc('gamecount');
					M('member')->where(array('id'=>$v['id']))->setInc('validgamecount');
					$i++;
				}*/	
                //判断升级信息
