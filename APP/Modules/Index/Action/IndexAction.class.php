<?php  
	...
	Class IndexAction extends CommonAction{
		...
		$info = M('jinbidetail')->field('FROM_UNIXTIME(addtime,"%Y-%m-%d") as datetime')->where($map)->order('id desc')->limit(20)->group('FROM_UNIXTIME(addtime,"%Y-%m-%d")')->select();
		foreach($info as $key=>$val){
			$date = $val['datetime'];
			//静态红包
			$j1 = M('jinbidetail')->where("FROM_UNIXTIME(addtime,'%Y-%m-%d') = '$date' and type=1 and `member`='$username'")->sum('adds');
			if(empty($j1)){
				$j1 = 0;
			}
			$info[$key]['j1'] = $j1;
			//分红红包
			$j2 = M('jinbidetail')->where("FROM_UNIXTIME(addtime,'%Y-%m-%d') = '$date' and type=2 and `member`='$username'")->sum('adds');
			if(empty($j2)){
				$j2 = 0;
			}			
			$info[$key]['j2'] = $j2;
			//合伙人红包
			$j3 = M('jinbidetail')->where("FROM_UNIXTIME(addtime,'%Y-%m-%d') = '$date' and type=3 and `member`='$username'")->sum('adds');
			if(empty($j3)){
				$j3 = 0;
			}				
			$info[$key]['j3'] = $j3;
			$j4 = M('jinbidetail')->where("FROM_UNIXTIME(addtime,'%Y-%m-%d') = '$date' and type=4 and `member`='$username'")->sum('adds');
			if(empty($j4)){
				$j4 = 0;
			}				
			$info[$key]['j4'] = $j4;
			$j5 = M('jinbidetail')->where("FROM_UNIXTIME(addtime,'%Y-%m-%d') = '$date' and type=5 and `member`='$username'")->sum('adds');
			if(empty($j5)){
				$j5 = 0;
			}				
			$info[$key]['j5'] = $j5;				
