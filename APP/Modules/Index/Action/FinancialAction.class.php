    ...
    public function kChange(){
        ...
        if (IS_POST) {
           ...
           **/			
            ...
            if ($jinbi < $outmoney) {
                  $this->ajaxReturn(array('info'=>'pv余额不足!')); 
            }

            //更新转出人余额
            $res1 = $member->where(array('username'=>$outusername))->setDec('jinbi',$outmoney);
            ...
            if($res1 && $res2){
				 $member->commit();
                 //转出人购物券明细
                 account_log3($outusername,$outmoney,'转出pv到编号'.$inusername,0);
                 //转入人购物券明细
		       	 account_log3($inusername,$outmoney,'收到编号为'. $outusername .'的购物券',1);				 
			}else{
				 $member->rollback();
			}
            //转出记录
