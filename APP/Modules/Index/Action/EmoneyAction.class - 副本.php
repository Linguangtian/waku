<?php  
	...
	public function wancheng(){
		...
		if($result['g_user']==$_SESSION['username']){
			...
			if($zz){
				$obs = M('member')->where(array('username'=>$result['g_user']))->setDec('qjinbi',$djmoney['qjinbi']);
				account_log4($result['g_user'],$djmoney['qjinbi'],'交易订单完成扣除',0);
	...
	public function cswancheng(){
		...
		if($result['p_user']==$_SESSION['username']){
			...
			$zz = M('ppdd')->where(array('g_user'=>$result['g_user'],'zt'=>1))->find();
			if($zz){
				$obs = M('member')->where(array('username'=>$result['p_user']))->setDec('qjinbi',$djmoney['qjinbi']);
				account_log4($result['p_user'],$djmoney['qjinbi'],'交易订单完成扣除',0);		
	...
	public function mjxx(){
		...
		$user=M('member')->where(array('username'=>$result['g_user']))->find();
		$user['rmb']=$rmb;
		$user['btc']=$btc;
	...
	public function maijia(){
		...
		$user = M('member')->where(array('username'=>$result['p_user']))->find();
		$user['rmb']=$rmb;
		$user['btc']=$btc;
		$user['meiyuan'] = $meiyuan;
...
public function myjiaoyis(){
	...
	public function cslkb(){
		...
		//交易密码
		$password_n = I("post.password_n",'',"trim");
		if(empty($password_n)){
			$this->ajaxReturn(array("status"=>2,"info"=>'请输入短信验证码！'));	
		}
		$user_id = session('mid');
		...
		$check_code = sms_code_verify($check_mobile,$password_n,session_id());				
		if($check_code['status'] != 1){
				$this->ajaxReturn(array('status'=>2,'info'=>$check_code['msg']));					
		}		
		...
		/*if(md5($password_n)!=$user['password2']){
			$this->ajaxReturn(array("status"=>2,"info"=>'交易密码错误！'));
		}
		*/
		...
		if($user['jinbi']<$totalprice){
			$this->ajaxReturn(array("status"=>2,"info"=>'出售的XYB不能大于您的余额'));
		}
		...
		account_log($gname,$totalprice,'XYB卖出挂单',0);		
		
		$csdec = M('member')->where(array('username'=>$gname))->setInc('qjinbi',$totalprice);
		account_log4($gname,$totalprice,'XYB卖出挂单',1);		
		$oob = $result->add($map);
		if($csdec){
			if($zdname!=""){
		...
		if(IS_AJAX){
			...
			if(false){
				...
				if($oob || $oob1){
					...
					$this->ajaxReturn(array("status"=>2,"msg"=>'您不能购买自己出售的XYB。'));
				}elseif($result['zt']==1){
					...
					if($re){
						...
						$this->ajaxReturn(array("status"=>1,"msg"=>'匹成功，请到我的交易中查看详情'));
					}
				}
			...
			if(false){
			   ...
			   $mobile_code = $data_P['mobile_code'];
				if(empty($mobile_code)){
					$this->ajaxReturn(array("status"=>2,"msg"=>'请输入短信验证码！'));	
				}
				$user_id = session('mid');
				$check_mobile=M('member')->where("id = {$user_id}")->getField('mobile');
				...
				if($user['jinbi'] < $totalprice){
					...
					$this->ajaxReturn(array("status"=>2,"msg"=>'您不能出售XYB到自己的账户'));					
				}elseif($result['zt']==1){
					$this->ajaxReturn(array("status"=>2,"msg"=>'对方正在交易中。'));					
				}elseif($result['zt']==2){
					$this->ajaxReturn(array("status"=>2,"msg"=>'对方交易已完成。'));					
		...
		public function withdraw(){
		   $user = M('member')->field("jinbi,point,truename,sktype,sknumber")->where(array('id'=>session('mid')))->find();
			if (IS_POST) {
				...
				$data['type'] = $user['sktype'];
				$data['account'] = $user['sknumber'];
