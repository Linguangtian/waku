	...
	//注册推广
		 public function regSempost(){
			 ...
			 if (IS_AJAX) {
				...
				//$data['wuliu']         = I('post.bduser','','htmlspecialchars');
				//$data['parent']        = I('post.parent','','htmlspecialchars');
				//$data['parent']        = I('post.parent','','intval');
                //$data['fl']            = $userinfo['fl'];
				//$data['nickname']      = '昵称';
				$data['username']      = $data['mobile']        = I('post.mobile','','strval');
				$code = I('post.code','');
			    //$data['truename']      = I('post.truename','','strval');
				//$data['idcard']        = I('post.idcard','','strval');
			    //$data['sktype']        = I('post.sktype','','strval');
				$data['weixin']      = I('post.weixin','','strval');
				$data['zhifubao']      = I('post.zhifubao','','strval');
				
				//$data['alipay_voucher']      = I('post.alipay_voucher','','strval');	
		...
		/**
				if(!preg_match("/^[a-zA-Z\d_]{6,}$/",$data['username'])){
					$this->ajaxReturn(array('info'=>'用户名不能小于6位!'));
				}	
				//验证会员编号是否存在
				if (M('member')->where(array('username'=>trim($data['username'])))->getField('id')) {
					$this->ajaxReturn(array('info'=>'用户名已存在，请更换！'));
				}
		
				if (!M('member')->where(array('username'=>$data['wuliu'],'isbaodan'=>1))->getField('id')) {
					$this->ajaxReturn(array('info'=>'对不起，报单中心不具备资格!'));
				}	
                **/				
				//验证推荐人信息是否已存在及审核
		...
		/**
				$choose =   I('post.choose','','strval'); //两侧
				$data['fparent']      = I('post.fparent','','htmlspecialchars');
				if(empty($data['fparent'])){
						$this->ajaxReturn(array('info'=>'请填写接点人!'));
				}				
					//安置人左右信息
				$fparentUser = M('member')->field('left,right,username')->where(array('username'=>$data['fparent'],'status'=>1))->find();		
				if(!$fparentUser){
					$this->ajaxReturn(array('info'=>'接点人不存在或未激活!'));
				}
				if (empty($choose) || $choose=='-1') {
					$this->ajaxReturn(array('info'=>'请选择接点区域!'));
				}			
         **/					

				/**
				if(empty($data['truename'])){
					$this->ajaxReturn(array('info'=>'请填写您的真实姓名!'));
				}	
				if(!preg_match("/^[\x{4e00}-\x{9fa5}]+$/u",$data['truename'])){
					$this->ajaxReturn(array('info'=>'真实姓名格式不正确!'));
				}	
				if(empty($data['idcard'])){
					$this->ajaxReturn(array('info'=>'请填写身份证号码!'));
				}	
				if(!preg_match("/^\d{17}(\d|x)$/",$data['idcard'])){
					$this->ajaxReturn(array('info'=>'身份证号码格式不正确!'));
				}	
				if(empty($data['sktype'])){
					$this->ajaxReturn(array('info'=>'请选择收款类型!'));
				}
				if(empty($data['sknumber'])){
					$this->ajaxReturn(array('info'=>'请填写收款账号!'));
				}	
                **/	
				
			/*	if(empty($data['truename'])){
					$this->ajaxReturn(array('result'=>0,'info'=>'请填写您的支付宝账户名!'));
				}	
				if(!preg_match("/^[\x{4e00}-\x{9fa5}]+$/u",$data['truename'])){
					$this->ajaxReturn(array('result'=>0,'info'=>'你的支付宝账户名格式不正确!'));
				}
				*/
